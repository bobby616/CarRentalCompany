...
export class CreateCaravan implements ICommand {
  ...
  public execute(parameters: string[]): string {
    const [passengerCapacity, pricePerDay, engine, color, transmissionType,
    ...
    if (isNaN(+passengerCapacity) || isNaN(+pricePerDay) || engine.length <= 0 ||
      ...
      transmissionType.localeCompare('Manual')) || (state.length <= 0) || brand.length <= 0 || isNaN(+livingArea) || isNaN(+numberOfBeds)
